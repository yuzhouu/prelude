...
export const ja = {
  ...
  topSites: {
    loading: 'よくアクセスするサイトを読み込み中…',
    empty: 'よくアクセスするサイトはありません',
    listDescription: 'Chrome が提供する順序で表示します。',
    accessError: '設定または権限を更新できませんでした。再試行してください。',
    denied: '権限が許可されなかったため、無効のままです。',
    unavailable:
      'Chrome 拡張機能で、よくアクセスするサイトを有効にしてください。',
    description:
      'よくアクセスするサイトをサイドバーと検索に表示します。初めて有効にするときに許可が必要です。',
    title: 'よくアクセスするサイト',
    hide: '「{{title}}」を非表示',
    restore: '非表示の項目を復元（{{count}}）',
    saveError: '非表示の設定を保存できませんでした。再試行してください。',
    privacy:
      '有効化と許可の後にのみ、Chrome のサイト名と URL を読み取り、サイドバーとローカル検索に使います。設定と非表示の URL は端末内に保存され、送信されません。',
