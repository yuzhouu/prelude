...
一个 Todoist 风格的 Chrome 新标签页扩展。它直接读取 Chrome 的书签树，并在侧栏和主内容区保留原有文件夹层级。

## 功能

- 使用 `chrome.bookmarks.getTree()` 读取真实书签与文件夹结构
- 书签发生新增、移动、重命名或删除时自动刷新
- 搜索标题、网址和所属文件夹路径；也可直接输入网址跳转
...
扩展声明 `bookmarks`、`favicon` 和 `tabs` 权限。`tabs` 用于读取当前标签页的标题、地址并切换标签；所有数据均在本地处理，没有远程数据服务。
