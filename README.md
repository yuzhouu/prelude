# 书签 · Chrome 新标签页

一个 Todoist 风格的 Chrome 新标签页扩展。它直接读取 Chrome 的书签树，并在侧栏和主内容区保留原有文件夹层级。

## 功能

- 使用 `chrome.bookmarks.getTree()` 读取真实书签与文件夹结构
- 在「快捷文件夹」页面确认后，于「其他书签」下创建「书签 · 新标签页」，内含可改名的「置顶」和「待读」文件夹
- 在每个书签文件夹下原位快捷添加书签
- 像 Todoist 添加分类一样，在每个文件夹下行内创建子文件夹
- 通过书签条目右侧菜单编辑标题与网址，或在确认后删除书签
- 添加时留空标题会优先读取已打开的同网址标签页标题；未命中时先使用域名，并在以后打开该网址时自动替换为真实页面标题。明确填写或手动修改的标题不会被覆盖
- 书签发生新增、移动、重命名或删除时自动刷新
- 搜索标题、网址和所属文件夹路径；也可直接输入网址跳转
- 使用 Chrome Extension Favicon API 显示站点图标
- 文件夹切换、最近添加、复制链接和窄屏抽屉导航
- 在侧边栏品牌区汇总显示全部书签的 Chrome 账号同步状态
- 按浏览器窗口展示当前打开的标签页，并可直接切换
- 普通浏览器开发环境使用示例书签，便于本地预览

## 本地开发

```bash
pnpm install
pnpm dev
```

## 安装到 Chrome

1. 构建扩展：

   ```bash
   pnpm build
   ```

2. 打开 `chrome://extensions/`。
3. 开启右上角「开发者模式」。
4. 点击「加载已解压的扩展程序」，选择本项目的 `dist` 目录。
5. 打开新标签页。扩展会请求书签读取权限，并用真实 Chrome 书签替换预览数据。

重新构建后，在 `chrome://extensions/` 中点击该扩展的刷新按钮即可更新。

## 数据与权限

扩展声明 `bookmarks`、`favicon`、`storage`、`tabs` 和 `webNavigation` 权限。`tabs` 用于读取当前标签页的标题、地址并切换标签；`storage` 只记录哪些书签仍在等待自动标题；`webNavigation` 用于在网址发生重定向时关联原始书签与最终页面标题，不读取页面正文。经用户确认创建的文件夹是普通 Chrome 书签文件夹，用户可以随时改名、移动或删除。所有数据均在本地处理，没有远程数据服务。
