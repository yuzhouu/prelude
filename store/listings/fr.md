# Texte de fiche Chrome Web Store en français

## Nom

Prelude

## Résumé

Un nouvel onglet calme pour organiser les favoris Chrome, les onglets ouverts et À lire plus tard.

## Description détaillée

```text
Un nouvel onglet calme pour organiser les favoris Chrome, les onglets ouverts et À lire plus tard.

Prelude fait du nouvel onglet Chrome le point de départ de votre navigation quotidienne. Ouvrez un site habituel, retrouvez un onglet dans une autre fenêtre ou reprenez une page enregistrée pour plus tard.

Prelude lit les favoris Chrome ainsi que les titres et URL des onglets ouverts afin de les afficher, ouvrir, rechercher, gérer et collecter. Les sites fréquemment visités ne sont consultés qu’après activation de la fonction et autorisation de votre part. Ces données restent locales : elles ne sont ni transférées, ni vendues, ni utilisées à des fins publicitaires. Seules les recherches Web envoyées explicitement par l'utilisateur sont confiées au moteur de recherche par défaut de Chrome.

Vos sites fréquents, si vous le souhaitez
Cette fonction est désactivée par défaut. Activez-la dans les paramètres et accordez l’autorisation pour retrouver vos sites fréquents dans la barre latérale et la recherche. Masquez des sites ou désactivez la fonction à tout moment. Prelude ne consulte pas l’ensemble de votre historique et ne compte pas les visites.

Vos favoris à portée de main
Conservez vos dossiers Chrome. Ajoutez, modifiez, supprimez et réorganisez les pages depuis un nouvel onglet. Aujourd’hui rassemble Épinglés, À lire plus tard et Favoris ; Ajoutés récemment vous aide à retrouver ce que vous venez d’enregistrer.

Reprenez votre navigation
Consultez les onglets de toutes vos fenêtres. Passez à une page, fermez un onglet terminé ou faites glisser les onglets pour les réorganiser ou les déplacer vers une autre fenêtre. Recherchez favoris et onglets ensemble. Ouvrez une adresse directement ou lancez une recherche Web avec le moteur par défaut de Chrome, sans modifier ses réglages.

Gardez une page pour plus tard
Depuis la barre d’outils ou le menu contextuel, enregistrez la page actuelle, une fenêtre entière ou un groupe d’onglets dans À lire plus tard. Le raccourci par défaut Alt + Shift + S enregistre la page actuelle. L’ordre est conservé et vous pouvez ignorer les URL en double ou garder une copie. L’option de fermeture après enregistrement ferme uniquement les onglets enregistrés avec succès. Les titres manquants sont complétés lorsque c’est possible, sans remplacer ceux que vous saisissez.

Selon vos habitudes
Choisissez un thème clair, sombre ou système. Disponible en chinois simplifié, anglais, japonais, espagnol, français et russe.

Confidentialité et contrôle
Ni compte, ni publicité, ni outil d’analyse. Vos favoris restent des favoris Chrome ordinaires et suivent vos réglages Chrome Sync. Les préférences et les données temporaires restent locales. Vos favoris et dossiers sont conservés après la désinstallation de Prelude, sauf si vous les supprimez.
```
